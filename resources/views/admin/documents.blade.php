...
<div class="min-h-screen bg-gradient-to-br from-gray-900 via-indigo-950 to-gray-900 text-white p-6">
    ...
    <div class="overflow-hidden rounded-lg bg-white/3 border border-white/10 min-h-[300px]">
            ...
            <tbody class="text-gray-200">
                @foreach($documents as $d)
                    <tr class="border-t border-white/5">
                        ...
                        <td class="px-6 py-4">{{ $d->user->name ?? $d->user->email }}</td>
                        <td class="px-6 py-4">{{ number_format($d->size/1024,2) }} KB</td>
                        ...
                        <td class="px-6 py-4">
                            <a href="{{ Storage::url($d->path) }}" class="mr-4 text-indigo-300">Télécharger</a>
                            <a href="{{ route('admin.compare', $d->id) }}" class="mr-4 text-indigo-300">Comparer</a>
                            <a href="{{ route('admin.reAnalyze', $d->id) }}" class="mr-4 text-yellow-300">Ré-analyser</a>
